...
let mode = 'pool';
let samples = [];            // hashrate history for the chart
...
setTimeout(async () => {
  ...
  if (s.autoStart && addr) {           // auto-start mining on open
    try { await StartMining(addr, mode); goDash(); } catch (e) { showSetup(); }
  } else {
    showSetup();
  }
}, 5000);

function showSetup(){ $('setup').style.display=''; $('dash').style.display='none'; }
function goDash(){ $('setup').style.display='none'; $('dash').style.display=''; }

/* ---------- setup ---------- */
$('continue').addEventListener('click', () => {
  const a = $('addr').value.trim();
  if (!/^0x[0-9a-fA-F]{40}$/.test(a)) { alert('Enter a valid LXS address (0x…), or press Create.'); return; }
  goDash();
});
...
$('start').addEventListener('click', async () => {
  const a = $('addr').value.trim();
  $('start').disabled = true;
  try { await StartMining(a, mode); } catch (e) { alert('' + e); }
...
setInterval(async () => {
  ...
  $('uptime').textContent = s.status === 'mining' || s.status === 'paused' ? '⏱ ' + s.uptime : '';
  ...
  $('start').style.display  = st === 'idle' ? '' : 'none';
  $('pause').style.display  = st === 'mining' ? '' : 'none';
  $('resume').style.display = st === 'paused' ? '' : 'none';
  $('stop').style.display   = st === 'idle' ? 'none' : '';
  document.querySelectorAll('.segbtn').forEach(b => b.style.opacity = st === 'idle' ? '1' : '.5');
